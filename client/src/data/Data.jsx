  ...
  const checkAccountLogin = async () => {
    ...
    try {
      ...
      if (response.data.authenticated) {
        await Promise.all([fetchCustomers(), fetchDrivers(), fetchVehicles(), fetchOrders()])
      } else {
        navigate('/signin')
      }
    } catch (error) {
    ...
    <div className="flex min-h-screen bg-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white border border-gray-200 p-6">
              ...
              <div className="text-2xl font-bold text-blue-600 mt-2">
                {orders.filter(o => o.orderType === 'import').length}
              </div>
            </div>
            <div className="bg-white border border-gray-200 p-6">
              <div className="text-sm text-gray-600">Export Orders</div>
              <div className="text-2xl font-bold text-green-600 mt-2">
                {orders.filter(o => o.orderType === 'export').length}
              </div>
            </div>
            <div className="bg-white border border-gray-200 p-6">
              <div className="text-sm text-gray-600">Total Revenue</div>
              <div className="text-2xl font-bold text-black mt-2">
                AED {orders.reduce((sum, o) => sum + parseFloat(o.total || 0), 0).toFixed(2)}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200">
            ...
            <div className="divide-y divide-gray-200">
              {filteredOrders.map((order) => (
                ...
                >
                  <div className="font-medium text-black">{order.containerNumber}</div>
                  <div className="text-gray-600">{order.from} → {order.to}</div>
                  <div className="text-gray-600">{getCustomerName(order.customerId)}</div>
                  <div className="text-gray-600">{getDriverName(order.driverId)}</div>
                  <div className="text-gray-600">{getVehicleInfo(order.vehicleId)}</div>
                  <div>
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      order.orderType === 'import' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                    }`}>
                      {order.orderType?.toUpperCase()}
                    </span>
                  </div>
                  <div className="font-medium text-black">AED {order.total}</div>
          ...
          <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit(handleAddOrder)} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Container Number *</label>
                  <input
                    type="text"
                ...
                <div className="relative driver-dropdown">
                  ...
                  <input type="hidden" {...register('driverId', { required: 'Driver is required' })} />
                ...
                <div className="relative vehicle-dropdown">
                  ...
                  <input type="hidden" {...register('vehicleId', { required: 'Vehicle is required' })} />
                  {errors.vehicleId && <p className="text-red-500 text-xs mt-1">{errors.vehicleId.message}</p>}
                </div>

                {/* Financial Fields */}
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Rate</label>
                  <input
                    type="number"
                    step="0.01"
                    {...register('rate')}
                ...
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Token</label>
                  <input
                    type="number"
                    step="0.01"
                    {...register('token')}
                ...
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Cust Wash</label>
                  <input
                    type="number"
                    step="0.01"
                    {...register('custWash')}
                ...
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Merc</label>
                  <input
                    type="number"
                    step="0.01"
                    {...register('merc')}
                ...
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Extra</label>
                  <input
                    type="number"
                    step="0.01"
                    {...register('extra')}
                ...
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Total</label>
                  <input
                    type="text"
      ...
      {showOrderDetail && selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="bg-black text-white px-6 py-4 flex justify-between items-center">
              <h3 className="text-lg font-medium">Order Details - {selectedOrder.containerNumber}</h3>
              <button 
            ...
            <div className="p-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedOrder.containerNumber}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Route:</span>
                      <span className="font-medium">{selectedOrder.from} → {selectedOrder.to}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Customer:</span>
                      <span className="font-medium">{getCustomerName(selectedOrder.customerId)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Driver:</span>
                      <span className="font-medium">{getDriverName(selectedOrder.driverId)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Vehicle:</span>
                      <span className="font-medium">{getVehicleInfo(selectedOrder.vehicleId)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Order Type:</span>
                      <span className={`px-2 py-1 rounded-full text-xs ${
                        selectedOrder.orderType === 'import' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                      }`}>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-4 space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">${selectedOrder.rate || '0.00'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Token:</span>
                      <span className="font-medium">${selectedOrder.token || '0.00'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Cust Wash:</span>
                      <span className="font-medium">${selectedOrder.custWash || '0.00'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Merc:</span>
                      <span className="font-medium">${selectedOrder.merc || '0.00'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Extra:</span>
                      <span className="font-medium">${selectedOrder.extra || '0.00'}</span>
                    </div>
                    <div className="border-t border-gray-200 pt-3">
                      <div className="flex justify-between">
                        <span className="font-semibold text-black">Total:</span>
                        <span className="font-bold text-black text-lg">${selectedOrder.total}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {selectedOrder.remarks && (
                <div className="mt-6">
                  ...
                  <p className="text-gray-700 bg-gray-50 p-4 rounded">{selectedOrder.remarks}</p>
                </div>
              )}
